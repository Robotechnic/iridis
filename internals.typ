...
#let colorize-math(equation, i: 0) = {
  ...
  if equation.func() == math.equation {
    ...
    math.frac(colorize-math(equation.num, i: i), colorize-math(equation.denom, i: i), ..copy-fields(equation, exclude: (
      "num",
      "denom",
    )))
  ...
  } else if equation.func() == math.vec {
    context {
      let color = text.fill
  ...
  } else if equation.func() == math.mat {
    context {
      let color = text.fill
      show: text.with(fill: iridis-palette.at(calc.rem(i, iridis-palette.len())))
      math.mat(
